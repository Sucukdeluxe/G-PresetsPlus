...
- **Preset editor** — the room settings of a preset in Habbo's own tab layout (Basic / Access / HC / ModTools), plus a **Floor plan** tab that edits the plan stored with the preset: type it, load it from a file, or remove it. Dimensions, walkable tiles and the derived door are shown while typing, and saving is blocked if the plan breaks the door rule or the size limits. Room categories are read live from the server, not hard-coded.
...
- **Furni are named the way Habbo names them** — *Gold Habbo Roller*, not `queue_tile1*4`. Across the whole catalogue that resolves for 17,090 of 17,113 floor items and 811 of 834 wall items; the rest are ad placeholders and generic types that carry no name in the furnidata at all, and fall back to the class name.
- Furni that do not end up in the room are named with tile, and the two reasons are kept apart: **not available** (nothing was sent, because there is no Builders Club offer and none in the inventory) and **refused by the server** — the latter being what Habbo shows as *"Sorry, you cannot place this item here"*.
- **The build repairs itself.** Refused drops are retried on the free work tile. Furni whose confirmation could not be matched — they stay lying on the drop tile and are invisible to both move passes — are paired back to their preset entry by type and moved into place. Only what still cannot be assigned is picked up, so the work area can be removed at all.
...
- **There are two independent size limits, and the client names both.** *"The maximum number of tiles usable in the floorplan editor is 3025 (55 x 55)"* and *"Width too large (max 64 tiles)"*. So a 64×24 plan is legal — 1536 tiles, 64 per side — while 56×56 is 3136 tiles and is not. Checking only the sides rejects wide flat builds that work; checking only the area lets through plans the server refuses.
- **The floor plan write can fail without any error.** Same family as `CreateFlat`: the server answers with the old plan, or not at all, and a moment later the identical clone runs through. The write is attempted three times, the room entry is waited out first, and every attempt logs the room size, the plan size, the model and which of the known rules the plan satisfies - so a real cause can be told apart from a hiccup.
- **The work area has to know about both.** It used to be appended to the right of the room unconditionally. On a plan that is already 64 wide that overflows the side limit, and the build dies on *Floor plan validation failed* after the room has been created. It now falls back to appending below.
